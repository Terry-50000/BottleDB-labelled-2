...
def bottle_counter(camera):
    ...
    while camera.IsGrabbing():
        ...
        if grabResult.GrabSucceeded():
            ...
            frame = cv2.copyMakeBorder(frame, 0, 1080 - frame_height, 0, 1720 - frame_width, cv2.BORDER_CONSTANT, value=(0,0,0))
            ...
            conv_img = cv2.resize(conv_img, (cv_width * (1040 - frame_height) // cv_height, 1040 - frame_height))
            frame[1100 - frame_height:1060, 20 :cv_width * (1040 - frame_height) // cv_height + 20] = conv_img
...
if __name__ == "__main__":
    ...
    pylon.FeaturePersistence.Load('Basler_config/4_10_2.pfs', camera.GetNodeMap(), True)
    ...
    yolo_name = "YOLOv8_models/best_from_best.pt"
    
    detector = YOLOv8_ObjectDetector(yolo_name, conf=0.35, iou=0.4, classes=[39])
